...
class Ask:
    ...
    async def prepare_images(self):
        if self.filepath:
            upload_result = await self.result.get_url()
            self.image_url = [
                item["data"]["url"]
                for item in upload_result
            ]

            self.json_data["message"]["files"] = [
                {
                    "url": url,
                    "mimeType": mimetypes.guess_type(self.filepath[i])[0] or "image/jpeg",
                    "name": self.filepath[i],
                }
                for i, url in enumerate(self.image_url)
            ]
    ...
    async def chat(self):
        await self.prepare_images()
        data = await self.final()
